...
* **確認已按照本指南在 `LICENSES/` 資料夾加入套件的授權檔案。**

> 本倉庫中的套件會直接被使用者透過 `apt` 安裝，因此請務必仔細檢查套件內容及其授權資訊。

## 授權資訊

每個新增或更新的套件，都必須在倉庫根目錄的：

```text
LICENSES/
```

資料夾中加入一個描述該套件授權方式的檔案。
...
例如，如果加入的組件名稱為 `cmd`，而其使用 MIT License，則應建立：
...
如果授權模式包含需要避免檔案名稱歧義的內容，請使用清楚、簡潔且容易辨識的授權名稱。

### 授權檔案內容

授權檔案至少應包含該組件所使用的授權資訊，以及必要的版權聲明。
...
如果上游專案已提供正式的 LICENSE 或 COPYING 文件，建議將其授權內容放入對應檔案，或在檔案中清楚標示其來源。

### 多重授權

如果一個組件同時使用多種授權，請在檔案名稱中清楚標示，例如：

```text
LICENSES/package_MIT_GPL-3.0
```

並在檔案內容中詳細列出各部分所適用的授權。
...
* 授權模式與上游專案的授權資訊一致。
* 沒有把不存在的授權名稱當成正式授權。
* 如果套件包含第三方程式碼，也需要檢查其授權。
* 不確定授權方式時，不要自行猜測，應先確認上游專案提供的 LICENSE、COPYING 或其他授權文件。

> **請不要因為程式碼可以免費下載或使用，就直接將其標示為 MIT、GPL 或其他開源授權。免費使用與開源授權是不同的概念。**
...
* [ ] 沒有提交私人金鑰、密碼或其他敏感資訊。
* [ ] 已檢查套件內所有檔案的內容。
* [ ] **已在 `LICENSES/` 加入符合 `組件名稱_授權模式` 格式的授權檔案。**
* [ ] **已確認授權檔案中的授權資訊與上游專案一致。**
* [ ] PR 說明中清楚描述了新增或修改的內容。
...
* **`LICENSES/` 中的授權資訊（如授權方式或版權聲明發生變更）。**
